...
class AlphabetSegmenter:
    ...
    def __init__(
        ...
        Args:
            ...
            min_char_size: Minimum character size in pixels
    ...
    def segment_auto_detect(
        ...
        for cnt in contours:
            ...
            if w < min_dot_size or h < min_dot_size or area < 25:
                continue
        ...
        if all_bboxes:
            ...
            # Filter out remaining small orphans
            bboxes = [b for b in merged
    ...
    def _segment_contour(
        ...
        bboxes = []
        small_bboxes = []  # Potential dots (too small to be standalone chars)
        min_dot_size = 5   # Minimum size to consider (reject noise)
        ...
        for cnt in contours:
            ...
            if w < min_dot_size or h < min_dot_size or area < 25:
                ...
                # Small component - could be a dot for 'i' or 'j'
                small_bboxes.append((x, y, w, h, cnt))
        ...
        # Combine for merging - dots will be merged with their stems
        all_bboxes = bboxes + small_bboxes
        ...
        # After merging, filter out any remaining small orphan components
        bboxes = [b for b in merged_bboxes
                  if b[2] >= self.min_char_size and b[3] >= self.min_char_size]
