...
Convert character images into animatable puppets for Adobe Character Animator with a guided wizard workflow.

**Wizard Steps:**
1. **Dependency Check** - Auto-install AI components on first use (~8-12GB)
2. **Image Selection** - Choose source image with preview and requirements guidance
3. **Body Part Detection** - AI-powered segmentation with bounding box visualization
4. **Viseme Generation** - Cloud AI creates 14 mouth shapes and eye blinks
5. **Export** - Save as PSD or SVG with Adobe-compatible layer naming

**AI Features:**
- Body segmentation using MediaPipe Pose/Face and SAM 2
- Cloud AI viseme generation via Google Gemini or OpenAI
- Provider selection with real-time cost estimation
- Cached generation to avoid redundant API calls

**Viseme Support:**
14 mouth shapes for natural lip-sync: Neutral, Ah, D, Ee, F, L, M, Oh, R, S, Uh, W-Oo, Smile, Surprised

**Export Formats:**
- **PSD** - Best for photorealistic puppets, full layer hierarchy
- **SVG** - Best for cartoon/flat styles, vector-based layers

### Font Generator

Create custom fonts from alphabet images with automatic character segmentation.

**Wizard Steps:**
1. **Image Upload** - Load alphabet image with character set selection
2. **Segmentation** - Automatic character detection with manual adjustment
3. **Character Mapping** - Verify and correct character labels
4. **Font Settings** - Configure family name, style, version, and smoothing
5. **Preview & Export** - Real-time preview and TTF/OTF export

**Segmentation Methods:**
- **Contour-based** - Automatic detection of individual characters
- **Grid-based** - Manual row/column specification for uniform grids
- **Auto Detect** - Intelligent selection based on image analysis

**Features:**
- Auto-inversion detection for light-on-dark text
- Multiple smoothing levels for vector quality control
- Support for uppercase, lowercase, digits, and custom character sets
- Font metadata (designer, copyright, version)
...
*Version 0.32.0 | [GitHub Repository](https://github.com/lelandg/ImageAI) | [Chameleon Labs Discord](https://discord.gg/chameleonlabs)*
