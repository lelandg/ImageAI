...
15. **`WorkerHost.shutdown()` keeps a timed-out worker alive** (final review 2026-08-30, Important 1). The prototype above drops the reference after the 5 s wait, which lets a second worker start and lets Qt destroy a running `QThread` at app exit (SIGABRT). Shipped behaviour: `shutdown()` returns `bool`; on timeout the worker is re-parented to `None`, kept in a host-level orphan list, and reaped (`wait()` + `deleteLater()`) when its own `finished`/`failed`/`cancelled` signal arrives; `is_busy()` stays True while an orphan runs; `join_orphans()` waits for them; `MainWindow.closeEvent` calls `join_orphans()` (unbounded) when `SpriteTab.shutdown()` returns False. `make_chroma_plate` and `generate_action_cards` take an optional `token` and check it around the provider call; `ActionCardsPanel` gains a Cancel button.
16. **`CharacterPanel` has no Ctrl+Enter primary action** (final review 2026-08-30, Minor 1). The panel has two equal actions (plate, turnaround); sub-project 5b's `install_shortcuts` decides the binding.
